...
void Search::Worker::iterative_deepening() {
    ...
    if (mainThread)
    {
        ...
        mainThread->stableCount = 0;
        mainThread->stablePV.clear();
    }
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        // Stability-based early termination (only during timed games;
        // never during "go infinite" or pondering per UCI protocol)
        if (mainThread && !threads.stop && bool(options["StabilityTermination"])
            && !limits.mate && !limits.infinite && !mainThread->ponder
            && limits.use_time_management()
            && rootDepth >= int(options["StabilityMinDepth"]))
        {
            int   threshold = int(options["StabilityThreshold"]);
            Value prevScore = rootMoves[0].previousScore;
            Value currScore = rootMoves[0].score;
            bool  decisive  = std::abs(currScore) > 600;

            int   delta     = std::abs(currScore - prevScore);
            int   base      = std::max(std::abs(currScore), 25);
            int   scaled    = delta * 100 / base;

            if (scaled <= threshold && prevScore != -VALUE_INFINITE)
                mainThread->stableCount++;
            else
                mainThread->stableCount = 0;

            // Save PV at first stable iteration — this is where the
            // engine found its answer, before confirmation overhead
            if (mainThread->stableCount == 1)
                mainThread->stablePV = rootMoves[0].pv;

            if (mainThread->stableCount >= 2)
            {
                // Restore the PV from the first stable iteration
                if (!mainThread->stablePV.empty())
                    rootMoves[0].pv = mainThread->stablePV;

                sync_cout << "info string Stability termination at depth "
                          << rootDepth << " (first stable at depth "
                          << (rootDepth - 1) << (decisive ? ", decisive" : "")
                          << ")" << sync_endl;
                threads.stop = true;
            }
        }
